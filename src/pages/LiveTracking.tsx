...
const LiveTracking = () => {
  ...
  const [useOlaMaps, setUseOlaMaps] = useState<boolean>(() => {
    return !!import.meta.env.VITE_OLA_MAPS_API_KEY;
  });
  ...
  useEffect(() => {
    ...
    if (useOlaMaps && !import.meta.env.VITE_OLA_MAPS_API_KEY) {
      // Ola Maps token missing: skip map initialization, UI will show fallback overlay
      return;
    }
    ...
    map.on('load', () => {
      ...
      map.addLayer({
        ...
        paint: {
          'line-color': '#111827', // Dark near-black for Zomato style
          'line-width': 5,
          'line-opacity': 0.8,
        },
      });
  ...
  }, [employees, loading, selectedId, useOlaMaps]);
    ...
    <div className={`h-full flex flex-col ${isMobile ? 'tracking-page-mobile' : ''}`}>
      ...
      <div className={`flex-1 ${isMobile ? 'relative -mx-4 -mt-2 -mb-4 md:-mx-6 md:-mb-6' : 'grid grid-cols-1 lg:grid-cols-4 gap-6 min-h-[600px] mt-2'}`}>
        ...
        <Card className={`${isMobile ? 'h-full border-0 shadow-none rounded-none' : 'lg:col-span-3 h-full border-border/50 shadow-sm'} overflow-hidden relative z-0`}>
          ...
          {/* Ola Maps / OpenStreetMap Floating Switcher */}
          <div className="absolute top-3 right-12 z-[10] flex gap-2">
              ...
              {/* Ola Maps Token Missing Overlay */}
              {useOlaMaps && !import.meta.env.VITE_OLA_MAPS_API_KEY && (
                <div className="absolute inset-0 z-[20] flex flex-col items-center justify-center bg-slate-950/65 backdrop-blur-sm text-center p-6 text-foreground">
                  <div className="bg-card/95 border border-border p-6 rounded-2xl max-w-md shadow-2xl flex flex-col items-center gap-4 animate-in fade-in zoom-in duration-200">
                    ...
                    <div className="space-y-1">
                      <h4 className="font-semibold text-base text-card-foreground">Ola Maps API Key Required</h4>
                      <p className="text-xs text-muted-foreground leading-relaxed px-2">
                        To render Ola Maps, you need to add your personal Ola Maps API Key.
                      </p>
                    </div>
                    ...
                    <div className="bg-muted/80 border border-border rounded-lg p-2.5 text-[11px] font-mono text-muted-foreground w-full select-all text-left">
                      VITE_OLA_MAPS_API_KEY=your_token_here
                    </div>

                    <p className="text-[10px] text-muted-foreground leading-normal">
                      Add this variable to your <code className="bg-muted px-1 py-0.5 rounded">.env</code> file.
                    </p>
                    ...
                    <div className="flex gap-2 w-full mt-2">
                      ...
                      <a
                        href="https://maps.olakrutrim.com/dashboard"
                        target="_blank"
                        rel="noopener noreferrer"
            ...
            {/* Small loading overlay if fetching data */}
            {loading && (
              <div className="absolute inset-0 z-[5] flex items-center justify-center bg-slate-50/50 backdrop-blur-sm pointer-events-none">
